...
match_test() ->
    T1 = retrie:insert_pattern("%{STRING:name}, helló!", p1, retrie:new()),
    ?assertEqual({p1, [{"name", "World"}]}, retrie:lookup_match("World, helló!", T1)),
    ?assertEqual(nomatch, retrie:lookup_match("World, World!", T1)),
    T11 = retrie:insert_pattern("Hello, %{STRING:name}", p11, T1),
    ?assertEqual({p11, [{"name", "helló"}]}, retrie:lookup_match("Hello, helló", T11)),
